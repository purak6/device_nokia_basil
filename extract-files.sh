...
HELPER="$LLUVIA_ROOT"/vendor/aosp/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
GOODIX="$LLUVIA_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib64/libgf_ca.so

sed -i "s|/system/etc/firmware|/vendor/firmware\x0\x0\x0\x0|g" $GOODIX

BLOB_ROOT="$LLUVIA_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary
